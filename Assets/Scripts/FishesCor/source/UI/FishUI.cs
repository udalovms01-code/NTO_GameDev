...
public class FishUI : MonoBehaviour
{
    ...
    public IEnumerator ScaleCountIn(Transform hungerValueTransform)
    {
        hungerValueTransform.transform.DOKill(true);
        yield return hungerValueTransform.transform.DOScale(Vector3.one * 1.2f, 0.01f).WaitForCompletion();
    }
    
    public IEnumerator ScaleCountOut(Transform hungerValueTransform)
    {
        hungerValueTransform.transform.DOKill(true);
        yield return hungerValueTransform.transform.DOScale(Vector3.one * 1f, 0.1f).WaitForCompletion();
    }
